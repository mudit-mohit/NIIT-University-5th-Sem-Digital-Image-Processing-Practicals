...
input_image = cv.imread('C:\Dev\DIP\Open CV\photos\Lenna.png')
...
cv.imwrite('salt_and_pepper_noisy_lena.jpg', noisy_img)
cv.imwrite('custom_median_filtered_lena.jpg', median_filtered_img)
# Displaying the original, noisy, and filtered images
cv.imshow('Original Image', input_image)
cv.imshow('Noisy Image', noisy_img)
cv.imshow('Custom Median Filtered Image', median_filtered_img)
cv.waitKey(0)
